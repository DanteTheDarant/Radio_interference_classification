clear
samples = 35; %number of samples of each SNR and amp level combi
...
for i = 1:missingSamples
    ...
    while (size(output,1) < numScans)
        ...
        payloadSize = randi([1,4])*1000;
        in = randi([0, 1], payloadSize, 1);
        

        ekstraPakker = randi(35);
        numPakker = 35+ekstraPakker;
        ...
        [tempOutput, label] = generateNonHTCSampling(nonHTCfg,channel,numPakker,in,idle,currentSNR,startChannel,amp);
