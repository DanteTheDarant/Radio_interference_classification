function [outputMatrix,labels] = generateHTCSampling(wlanConfig,wifiChannel,numPackets,payload,idle_time,snr,startChannel,mixAmplitude)
    %% SETUP THINGS
    %clear
    if (wifiChannel > 12)
        exit;
    ...
    Tsplit = 0.1; %time per split in processing, lower to decrease RAM needed. Higher speeds up the calculations a lot
    ...
    if (htCfg.ChannelBandwidth == 'CBW20')
        oversampling = 8;
        labelWidth = 20;
    elseif (htCfg.ChannelBandwidth == 'CBW40')
        oversampling = 4;
        labelWidth = 40;
    elseif (htCfg.ChannelBandwidth == 'CBW10')
        oversampling = 16;
        labelWidth = 10;
    ...
    waveform = wlanWaveformGenerator(payload, htCfg, ...
        'NumPackets', 8, ...
        'IdleTime', 0, ...
    ...
    Fs = wlanSampleRate(htCfg, 'OversamplingFactor', oversampling);     
    ...
    % TODO: multiple different signals in one function
    %tic
    Tmax = length(waveform)/Fs;
    ...
    %     t = t(samplesInSplit+1:end);
        ...
        mixedWLAN = [mixedWLAN ; awgn(current_wave.*mixerSine, snr, levelForNoise)];
